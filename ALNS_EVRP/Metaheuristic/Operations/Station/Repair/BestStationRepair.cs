...
{
    public class BestStationRepair : RepairOperation
    {
        public BestStationRepair(int allOps) : base(allOps) { }
        ...
        public override Solution Repair(Solution sol, int requestBankSelected = 0)
        {
            ...
            for (int r = 0; r < sol.Routes.Count; r++)
            {
                ...
                while (n > 0 && sol.Routes[r].Nodes[n].Type != TypeNode.Station)
                {
                    ...
                    foreach (Node station in sol.Stations)
                    {
                        ...
                        foreach (Speed speedSet in sol.Routes[r].Vehicle.Speeds)
                        {
                            ...
                            // If the new route cost inserting the station in route r at index n is better than the best
                            // and the solution is feasible inserting the station, I save the position and the best new cost
